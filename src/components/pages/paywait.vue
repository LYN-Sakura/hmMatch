...
export default {
    ...
    methods:{
        gettype(){
            const waittype  = setInterval(()=>{
                this.$http.post('mobile/order/query',{
                        ...
                        if(res.data.status==1){
                            Toast('支付成功!')
                            this.$router.push('/paystatus?ordersn='+this.ordersn+'&status=1'+'&token='+this.token)
                        }else{
                            this.$router.push('/paystatus?ordersn='+this.ordersn+'&status=0'+'&token='+this.token)
                        }
                    } )
            },3000)
    ...
    created(){
        ...
        this.token=this.$route.query.token //sessionStorage.getItem("ms_token")
